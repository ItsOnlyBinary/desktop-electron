...
(function () {
    const root = this;
    const previous_emoji = root.EmojiConvertor;
    ...
    emoji.prototype.replace_emoticons = function (str) {
        const self = this;
        const colonized = self.replace_emoticons_with_colons(str);
    ...
    emoji.prototype.replace_emoticons_with_colons = function (str) {
        ...
        self.init_emoticons();
        let _prev_offset = 0;
        const emoticons_with_parens = [];
        let str_replaced = str.replace(self.rx_emoticons, function (m, $1, emoticon, offset) {
            const prev_offset = _prev_offset;
            _prev_offset = offset + m.length;

            const has_open_paren = emoticon.indexOf('(') !== -1;
            const has_close_paren = emoticon.indexOf(')') !== -1;
            ...
            if (has_close_paren && !has_open_paren) {
                const piece = str.substring(prev_offset, offset);
            ...
            /*
                ...
                */
            if (m === '\n8)') {
                const before_match = str.substring(0, offset);
                if (/\n?(6\)|7\))/.test(before_match)) return m;
            }

            const val = self.data[self.map.emoticons[emoticon]][3][0];
            return val ? $1 + ':' + val + ':' : m;
        });
        ...
        if (emoticons_with_parens.length) {
            const escaped_emoticons = emoticons_with_parens.map(self.escape_rx);
            const parenthetical_rx = new RegExp('(\\(.+)(' + escaped_emoticons.join('|') + ')(.+\\))', 'g');
    ...
    emoji.prototype.replace_colons = function (str) {
        ...
        return str.replace(self.rx_colons, function (m) {
            ...
            if (idx.indexOf('::skin-tone-') > -1) {
                const skin_tone = idx.substr(-1, 1);
                const skin_idx = 'skin-tone-' + skin_tone;
                const skin_val = self.map.colons[skin_idx];
    ...
    emoji.prototype.replacement = function (idx, actual, wrapper, variation, is_extra) {
        ...
        const text_name = (actual) ? wrapper + actual + wrapper : self.data[idx][8] || wrapper + self.data[idx][3][0] + wrapper;
        ...
        const img = self.find_image(idx, var_idx);
        ...
        const title = self.include_title ? ' title="' + (actual || self.data[idx][3][0]) + '"' : '';
        ...
        if (self.supports_css) {
            if (self.use_sheet && img.px != null && img.py != null) {
                const sheet_size = self.sheet_size * (img.sheet_size + 2); // size of image in pixels
                ...
                const style = 'background: url(' + img.sheet + ');background-position:' + (sheet_x) + '% ' + (sheet_y) + '%;background-size:' + sheet_sz + '% ' + sheet_sz + '%';
    ...
    emoji.prototype.find_image = function (idx, var_idx) {
        ...
        const try_order = [self.img_set, 'apple', 'emojione', 'google', 'twitter', 'facebook', 'messenger'];

        // for each image set, see if we have the image we need. otherwise check for an obsolete in
        // that image set
        for (let j = 0; j < try_order.length; j++) {
            ...
            if (self.obsoletes_data[out.full_idx]) {
                const ob_data = self.obsoletes_data[out.full_idx];
    ...
    emoji.prototype.init_emoticons = function () {
        ...
        const a = [];
        self.map.emoticons = {};
        for (const i in self.emoticons_data) {
            // because we never see some characters in our text except as entities, we must do some replacing
            const emoticon = i.replace(/\&/g, '&amp;').replace(/\</g, '&lt;').replace(/\>/g, '&gt;');
    ...
    emoji.prototype.init_unified = function () {
        ...
        for (var i in self.variations_data) {
            ...
            for (const k in self.variations_data[i]) {
